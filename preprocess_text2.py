...
def main(
    ...
    if clean:
        ...
        for line in tqdm(open(metadata, encoding="utf-8").readlines()):
            try:
                utt, spk, input_audio, language, emo, input_emo,goal_emo_path,input_emo_path,text = line.strip().split("|")
    ...
    with open(metadata, encoding="utf-8") as f:
        for line in f.readlines():
            print('line',line)
            # input()
            utt, spk, input_audio, language, emo, input_emo, goal_emo_path,input_emo_path,text, phones, tones, word2ph = line.strip().split("|")
    ...
    for emo, utts in spk_utt_map.items():
        shuffle(utts)
