...
def load_checkpoint_old(checkpoint_path, model, optimizer=None, skip_optimizer=False):
...
def load_checkpoint(checkpoint_path, model, optimizer=None, is_old=False,skip_optimizer=False):
    assert os.path.isfile(checkpoint_path)
    checkpoint_dict = torch.load(checkpoint_path, map_location='cpu')
...
def save_se(g,se_path):
    # print('enter se')
    logger.info(
        "Saving speaker embedding to {}".format(
            se_path
        )
    )
    torch.save(
...
def latest_checkpoint_path(dir_path, regex="G_*.pth"):
    ...
    x = f_list[-1]
    return x
def RL_pretrain_checkpoint_path(dir_path, regex="G_*.pth"):
    f_list = glob.glob(os.path.join(dir_path, regex))
    f_list.sort(key=lambda f: int("".join(filter(str.isdigit, f))))
    x = f_list[-1]
    return x
...
def load_filepaths_and_text(filename, split="|"):
    with open(filename, encoding="utf-8") as f:
        ...
        # print('filepaths_and_text',filepaths_and_text)
        # input()
...
def get_hparams(init=True):
    ...
    model_dir = os.path.join("./logs2", args.model)
    print('model_dir',model_dir)
    pretrained_RL_dir = os.path.join("./pretrained_RL_emo", args.model)


    os.makedirs(model_dir, exist_ok=True)
    os.makedirs(pretrained_RL_dir, exist_ok=True)
    ...
    hparams.pretrained_RL_dir = pretrained_RL_dir
    ...
    # hparams.pretrain_rl = args.pretrain_rl
