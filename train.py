...
def train_and_evaluate(
    ...
    for batch_idx, data in enumerate(tqdm(train_loader)):
        ...
        # emo_emb=net_g.module.extract_se(inputaudio,hps,language)
        # emo_emb_ref=net_g.module.extract_se(audiopath,hps,language)
        print('outside of emo_extract')
        #print('emo_emb',emo_emb)
        ...
        with autocast(enabled=hps.train.fp16_run):
            print('outside of autocast436')
            
            print('enter train and eval train loader1629')
            # print(f"x input449: {x}")
            ...
            with torch.amp.autocast(device_type='cuda',enabled=False):
                print('outside of autocast492')
                
                loss_disc, losses_disc_r, losses_disc_g = discriminator_loss(
                    y_d_hat_r, y_d_hat_g
                )
                print('outside of autocast508')
            ...
            if net_dur_disc is not None:
                print('outside of autocast515')
                y_dur_hat_r, y_dur_hat_g = net_dur_disc(
                    hidden_x.detach(), x_mask.detach(), logw.detach(), logw_.detach()
                )
                print('outside of autocast519')
  
                with torch.amp.autocast(device_type='cuda',enabled=False):
                    print('outside of autocast505')
...
def evaluate(rank,hps, generator, eval_loader, writer_eval):
    ...
    with torch.no_grad():
        ...
        # input()
            ...
            # emo_emb=generator.module.extract_se(audiopath,hps,language)
